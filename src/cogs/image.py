...
async def motivational(ctx, args):
    ...
    try:
        ...
        draw.rectangle((margin - padding, margin - padding, width - margin + padding, height + padding - margin * 3), fill=None, outline="white", width=int(imwidth / 200))
        background.paste(image, (margin, margin))
        # draw the inside
        draw.text((x, y), text, font=font, fill="white")
        background.save(tempString)
        await ctx.message.reply(file=discord.File(tempString))
    except Exception:
        await ctx.send("An error occurred processing this image. ")
    finally:
...
class Image(commands.Cog):
    ...
    async def rotate(self, ctx, *, degrees):
        ...
        th.start()
    @commands.cooldown(3, 10, commands.BucketType.user)
    @commands.command(name="rmirror", description="replaces the left half of the image with a mirror image of the right half")
    async def rflip(self, ctx):
        th = threading.Thread(target=async_handler(reflectR, ctx, []))
        th.start()
    @commands.cooldown(3, 10, commands.BucketType.user)
    @commands.command(name="lmirror", description="replaces the right half of the image with a mirror image of the left half")
    ...
    async def scott(self, ctx):
        ...
        th.start()
    @commands.cooldown(3, 10, commands.BucketType.user)
    @commands.command(name="nintendo", description="Puts your image into a nintendo direct")
